...
Shader "Anty/Water/SimpleWater_step1"
{
	Properties
	{
	...
	SubShader
	{
		pass
		{
			...
			param CalculateParam(float4 vertex)
			{
				...
				float u = dot(d.xz,vertex.xz) * w - _Time.y * psi;		//之所以要用 - 而不是 + ，是为了模拟波源的运动要先于其他波的运动
			...
			float4 frag(v2f i):SV_TARGET
			{
				...
				// fixed4 diffuse = fixed4(-worldPos.y/A,worldPos.y/A,0,1);
...
//		step 1_1 : 法线计算，diffuse、specular			法线计算由于向量偏导的求解无法解决，暂时搁置,向后继续
